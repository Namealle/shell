...
the middle half of the pass) — no new slot, no new uniform, no shader change. `starBirth`/`nova`/`redGiant`/`supernova`/`kilonova`/`pulsar`/`gammaBurst` and `phenomena.tde` are validated and forwarded but NOT yet drawn.
