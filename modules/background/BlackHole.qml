...
QtObject {
    ...
    property var disk: _preset.disk !== undefined ? _preset.disk : ({})
    property var photon: _preset.photon !== undefined ? _preset.photon : ({})
    ...
    readonly property vector4d bhDetail: Qt.vector4d(_strengths[0], Math.round(value(disk, "seed", 457, 0, 65535)), value(disk, "rotationSign", 1, -1, 1) < 0 ? -1 : 1, Math.round(value(disk.streaks, "octaves", 2, 1, 3)))
    readonly property vector4d bhStreaks: Qt.vector4d(value(disk.streaks, "radialScale", 1, 0.5, 2), Math.round(value(disk.streaks, "innerCyclesPer4096", 32, 16, 128)), _strengths[1], _strengths[2])
    readonly property vector4d bhKnots: Qt.vector4d(value(disk.knots, "density", 0.03, 0, 0.06), _strengths[3], 16, 48)
    readonly property vector4d bhEmbers: Qt.vector4d(2 * Math.min(bhStreaks.y - 1, Math.floor(value(disk.embers, "count", 24, 0, 32) / 2)), value(disk.embers, "radiusRs", 0.012, 0.004, 0.025), value(disk.embers, "trailSec", 0.35, 0, 0.6), _strengths[4])
    readonly property vector4d bhDoppler: Qt.vector4d(_physical ? 1 : 0, _strengths[5], 0, 0)
    readonly property vector4d bhHue: Qt.vector4d(Math.log(value(disk.hue, "innerTemperature", 6500, 4200, 10000)), Math.log(value(disk.hue, "outerTemperature", 1700, 1000, 2800)), _strengths[6], _strengths[13])
    readonly property vector4d bhGlow: Qt.vector4d(_strengths[7], value(disk.glow, "radiusPx", 1.5, 0.25, 2.5), 0, 0)
    readonly property vector4d bhPhoton: Qt.vector4d(_strengths[8], _strengths[9], _strengths[10], photon.mode === "off" ? 0 : 1)
    readonly property vector4d bhDetailPhase: Qt.vector4d(_detailPhase, 1 / 30, 0, 0)
    readonly property vector4d bhArcs: Qt.vector4d(_strengths[12], value(disk.arcs, "radiusRh", 1.75, 1.2, 2.6), value(disk.arcs, "spacingRh", 0.6, 0.2, 1), Math.round(value(disk.arcs, "count", 2, 0, 2)))
    readonly property bool _physical: !!disk.doppler && disk.doppler.preset === "physical"
    ...
    function strengthTargets(): var {
        return [value(disk, "detail", 0.75, 0, 0.85), value(disk.streaks, "warp", 0.25, 0, 0.25), value(disk.streaks, "grain", 0.06, 0, 0.08), value(disk.knots, "gain", 0.35, 0, 0.5), value(disk.embers, "gain", 0.06, 0, 0.06), value(disk.doppler, "strength", _physical ? 1 : 0.22, 0, 1), value(disk.hue, "warmth", 0.5, 0, 1), value(disk.glow, "gain", 0.008, 0, 0.008), value(photon, "widthPx", 0.45, 0.1, 0.75), value(photon, "gain", 1.18, 0, 1.5), value(photon, "textureStrength", 0.8, 0, 1), value(disk, "exposure", 1, 0.5, 2), value(disk.arcs, "gain", 0, 0, 0.02), value(disk.hue, "whiteness", 0, 0, 1)];
    }
