...
var TAU = {cpuLoad: 15, cpuHeat: 60, gpuLoad: 20, gpuHeat: 60, vram: 30, ram: 30,
    network: 20, rain: 180, wind: 180, humidity: 180, temperature: 180, weatherNight: 180,
    media: 30, idle: 60, agentProcess: 60, night: 300, notifications: 20};
var MAX_AGE = {cpuLoad: 10, cpuHeat: 20, gpuLoad: 20, gpuHeat: 20, vram: 20, ram: 20,
    network: 20, rain: 10800, wind: 10800, humidity: 10800, temperature: 10800, weatherNight: 10800,
    media: 5, idle: 5, agentProcess: 30, night: 120, notifications: 5};

function finite(x) { return typeof x === "number" && isFinite(x); }
function clamp(x, a, b) { return Math.max(a, Math.min(b, x)); }
function smoothstep(a, b, x) { var t = clamp((x - a) / (b - a), 0, 1); return t * t * (3 - 2 * t); }
function ratio(x) { return finite(x) && x >= 0 && x <= 1 ? x : null; }
function heat(x, high) { return finite(x) && x > 0 && x <= 150 ? smoothstep(45, high, x) : null; }
...
function night(hours, minutes) {
    if (!finite(hours) || !finite(minutes)) return null;
...
function weather(cc) {
    ...
    if (!cc || codes.indexOf(cc.weatherCode) === -1) return result;
    var code = cc.weatherCode, rain = 0;
    if ([51, 53, 55, 56, 57].indexOf(code) !== -1) rain = 0.25;
    else if ([61, 66, 80].indexOf(code) !== -1) rain = 0.40;
    else if ([63, 81].indexOf(code) !== -1) rain = 0.65;
    else if ([65, 67, 82].indexOf(code) !== -1) rain = 0.90;
    else if ([95, 96, 99].indexOf(code) !== -1) rain = 0.80;
    result.rain = rain;
    if (finite(cc.windSpeed) && cc.windSpeed >= 0) result.wind = clamp(cc.windSpeed / 40, 0, 1);
    if (finite(cc.humidity) && cc.humidity >= 0 && cc.humidity <= 100) result.humidity = cc.humidity / 100;
    if (finite(cc.tempC)) result.temperature = clamp((cc.tempC + 10) / 45, 0, 1);
    if (cc.isDay === 0 || cc.isDay === 1) result.weatherNight = 1 - cc.isDay;
    return result;
}
function parseGpu(line) {
    ...
    var values = [];
    for (var i = 0; i < 4; i++) {
        if (!/^\s*\d+(?:\.\d+)?\s*$/.test(fields[i])) return null;
        values.push(Number(fields[i]));
        if (!finite(values[i])) return null;
    }
    if (values[0] <= 0 || values[0] > 150 || values[1] > 100 || values[3] <= 0 || values[2] > values[3]) return null;
    return {gpuHeat: heat(values[0], 80), gpuLoad: values[1] / 100, vram: values[2] / values[3]};
}
function parseCpu(text) {
    ...
    for (var i = 0; i < 8; i++) {
        if (!/^\d+$/.test(fields[i]) || !finite(Number(fields[i]))) return null;
        total += Number(fields[i]);
    }
    return {total: total, idle: Number(fields[3]) + Number(fields[4])};
}
...
function parseMemory(text) {
    ...
    if (!total || !available || Number(total[1]) <= 0 || Number(available[1]) > Number(total[1])) return null;
...
function elapsedStep(previousMono, mono, previousWall, wall) {
    ...
    return {dt: reset ? 0 : dt, reset: reset, ageAdvance: wallGap > 2 ? wallGap : 0};
}

function createBank() { return Object.create(null); }
function record(bank, name, raw, now, tau, maxAge) {
    if (!finite(raw) || !finite(now)) return false;
    var state = bank[name];
    if (!state) {
        state = {value: 0, velocity: 0, raw: 0, lastSeen: now, tau: tau || TAU[name] || 45,
...
function availability(lastSeen, now, maxAge) { return clamp(1 - Math.max(0, now - lastSeen - maxAge) / 120, 0, 1); }
...
function rate01(velocity) { return 0.5 + 0.5 * clamp(velocity / 0.03, -1, 1); }
function advance(bank, now, dt) {
    ...
    return {values: values, rates: rates, valid: valid, weights: weights};
}
function reset(bank) {
    var keys = Object.keys(bank);
    for (var i = 0; i < keys.length; i++) bank[keys[i]].velocity = 0;
}
function derive(values, processWeight, bank, weights) {
    function value(name) { return finite(values[name]) ? values[name] : 0; }
